...
function* fetchPackageGroupsSaga(): SagaIterator {
  try {
    ...
    yield put(
      fetchPackageGroupsFailure(error.message || "Something went wrong")
    );
  }
}

// Fetch single package group
function* fetchPackageGroupSaga(action: {
  type: string;
  payload: string;
}): SagaIterator {
  try {
...
function* createPackageGroupSaga(action: {
  type: any;
  payload: any;
}): SagaIterator {
  try {
    ...
    yield put(
      createPackageGroupFailure(
        error.message || "Failed to create package group"
      )
    );
  }
}
...
// Edit package group
function* editPackageGroupSaga(action: {
  type: any;
  payload: any;
}): SagaIterator {
  try {
    ...
    yield put(
      editPackageGroupFailure(error.message || "Failed to edit package group")
    );
  }
}

// Delete package group
function* deletePackageGroupSaga(action: {
  type: string;
  payload: string;
}): SagaIterator {
  try {
    ...
    yield put(
      deletePackageGroupFailure(
        error.message || "Failed to delete package group"
      )
    );
  }
}
